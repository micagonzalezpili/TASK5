...
const app = createApp({
    ...
    created() { // LO QUE EJECUTO MIENTRAS LA APP ESTE CREADA
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                this.categories = Array.from(new Set(this.events.map(events => events.category)))
    ...
    computed: {
        filtrosCombinados() {
            let aux = this.events.filter(event => event.name.toLowerCase().includes(this.nombreIngresado.toLowerCase()))
